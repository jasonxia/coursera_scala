...
object Huffman {
  ...
  def times(chars: List[Char]): List[(Char, Int)] = {
    def occurs(account: Map[Char,Int], char: Char) = {
      val count = (account get char).getOrElse(0) + 1
      account + ((char, count))
    }
    (Map[Char, Int]() /: chars)(occurs).iterator.toList
  }
  ...
  def until[T](test: (List[T]) => Boolean,
               func: (List[T]) => List[T])(trees: List[T]): List[T] = {
    if( test(trees) ) trees
    else until(test, func)(func(trees))
  }
...
object Main extends App {
  ...
  val encode1 = encode(t1)(string2Chars("abd"))
  println( encode1 )
